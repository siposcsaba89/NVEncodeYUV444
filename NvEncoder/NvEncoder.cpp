...
bool CNvEncoder::encodeFrame(CUdeviceptr input_img, NvEncodeImageType img_type, int cam_idx, int frame_idx)
{
    EncodeFrameConfig stEncodeFrame;
    memset(&stEncodeFrame, 0, sizeof(stEncodeFrame));
    stEncodeFrame.input_ptr = input_img;
    stEncodeFrame.img_type = img_type;
    ...
    for (size_t j = 0; j < m_EncodeBufferQueue.size(); ++j)
    {
        ...
        for (uint32_t i = 0; i < m_uEncodeBufferCount; i++)
        {
            ...
            cuMemAllocPitch((CUdeviceptr *)&m_stEncodeBuffer[j][i].stInputBfr.pNV12devPtr,
                &m_stEncodeBuffer[j][i].stInputBfr.dwPitch,
                uInputWidth,
...
NVENCSTATUS CNvEncoder::ReleaseIOBuffers()
{
    for (int j = 0; j < int(m_stEncodeBuffer.size()); ++j)
    {
        for (uint32_t i = 0; i < m_uEncodeBufferCount; i++)
        {
            ...
            cuMemFree(m_stEncodeBuffer[j][i].stInputBfr.pNV12devPtr);
            m_stEncodeBuffer[j][i].stInputBfr.pNV12devPtr = NULL;
            m_stEncodeBuffer[j][i].stInputBfr.hInputSurface = NULL;
            m_stEncodeBuffer[j][i].stInputBfr.nvRegisteredResource = NULL;
...
NVENCSTATUS CNvEncoder::EncodeFrame(EncodeFrameConfig *pEncodeFrame, bool bFlush, uint32_t width, uint32_t height, int cam_idx)
{
    ...
    { 
        cuMemcpy(pEncodeBuffer->stInputBfr.pNV12devPtr, pEncodeFrame->input_ptr, width * height + width * height / 2);
    }
