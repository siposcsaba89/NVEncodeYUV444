                                   ...
                                   char u, char v)
{
    ...
    kernel_convertBayer8ToNV12Y << <glob_dim, loc_dim >> > ((const uint8_t*)src, (uint8_t*)dst, width, height, pitch);
    kernel_convertBayer8ToNV12Y_setUV << <glob_dim, loc_dim >> > ((uint8_t*)dst, width, height, pitch, u);
}
...
void nvencc::convertNV12_Y_toBayer8(const CUdeviceptr src, CUdeviceptr dst, int32_t width, int32_t height, int32_t pitch)
{
    dim3 loc_dim(16, 16);
    dim3 glob_dim((width / 2 + loc_dim.x - 1) / loc_dim.x, (height / 2 + loc_dim.y - 1) / loc_dim.y);
    kernel_convertNV12YtoBayer8 << <glob_dim, loc_dim >> > ((const uint8_t*)src, (uint8_t*)dst, width, height, pitch);
}
